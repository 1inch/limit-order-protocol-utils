...
class LimitOrderBuilder {
    ...
    buildLimitOrderHash(orderTypedData) {
        ...
        return (
            limit_order_protocol_const_1.ZX +
            eth_sig_util_1.TypedDataUtils.sign(message).toString('hex')
        );
    }
    buildLimitOrderTypedData(
        order,
        domainName = limit_order_protocol_const_1.PROTOCOL_NAME
    ) {
        return {
            primaryType: 'Order',
            types: {
    ...
    buildRFQOrder({
        ...
        takerAssetAddress,
        makerAddress,
        takerAddress = limit_order_protocol_const_1.ZERO_ADDRESS,
        makerAmount,
        takerAmount,
    }) {
        return {
    ...
    buildLimitOrder({
        ...
        takerAssetAddress,
        makerAddress,
        receiver = limit_order_protocol_const_1.ZERO_ADDRESS,
        takerAddress = limit_order_protocol_const_1.ZERO_ADDRESS,
        makerAmount,
        takerAmount,
        predicate = limit_order_protocol_const_1.ZX,
        permit = limit_order_protocol_const_1.ZX,
        interaction = limit_order_protocol_const_1.ZX,
    }) {
        return {
